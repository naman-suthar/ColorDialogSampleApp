...
class ColorDialog : DialogFragment(),
    ...
    override fun onCreateView(
        ...
        Log.d("ColorHistoryCD","$colorHistory")
        colorHistory?.let { it() }?.let {
            dialogTabLayout.addTab(dialogTabLayout.newTab().setCustomView(R.layout.history_tab))
        }
    ...
    internal inner class ViewPagerAdapater(fm: FragmentManager?) :
        ...
        override fun getItem(position: Int): Fragment {
            when (position) {
                ...
                3 -> return HistoryTabFragment.newInstance(colorHistory, this@ColorDialog)
            }
            return Fragment()
        }

        override fun getCount(): Int {
            return if (colorHistory != null) 4 else 3
        }
    }
